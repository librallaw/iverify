<html>
<head>

    <?php
    $nonimage = "";



    $qrcode = "data:image/png;base64,";





    $nin = 54634782813;


    $first_nin = substr($nin, 0, 4);
    $second_nin = substr($nin, 4, 3);
    $third_nin = substr($nin, 7, 4);

    ?>







</head>


<style>
    .rotate1 {
        writing-mode: vertical-lr;
        -webkit-transform: rotate(-40deg);
        -moz-transform: rotate(-40deg);
        color:#a7b1a1;
        position: absolute;
        margin-top:165px;
        margin-left:20px;
        font-size: 15px;
        letter-spacing: 1px;
    }


    .rotate2 {
        writing-mode: vertical-lr;
        -webkit-transform: rotate(-35deg);
        -moz-transform: rotate(-35deg);
        color:#a7b1a1;
        position: absolute;
        margin-top:200px;
        margin-left:18px;
        font-size: 15px;
        letter-spacing: 1px;
    }


    .rotate3 {
        writing-mode: vertical-lr;
        -webkit-transform: rotate(35deg);
        -moz-transform: rotate(35deg);
        color:#a7b1a1;
        position: absolute;
        margin-top:210px;
        margin-left:360px;
        font-size: 13px;

    }



    .rotate4 {
        writing-mode: vertical-lr;
        -webkit-transform: rotate(-180deg);
        -moz-transform: rotate(-180deg);
        color:#a7b1a1;
        position: absolute;
        margin-top:140px;
        margin-left:315px;
        font-size: 13px;
        letter-spacing: 2px;

    }


</style>
<div class="id-card-tag" style="width:430px">




    <div style="position: absolute">
        <img src="http://iverify.test/image-001.jpg" style="width:90px;margin-top: 55px;margin-left: 10px">
    </div>
    <div  id="flip" class="rotate1">
        12345678901
    </div>
    <div  id="flip2" class="rotate2">
        22345678901
    </div>

    <div  id="flip3" class="rotate3">
        22345678901
    </div>

    <div  id="flip4" class="rotate4">
        22345678901
    </div>

    <div style="position: absolute;margin-top:5px;">
        <div style="margin-left: 120px;margin-top: 55px">
            <div>
                <div style="font-size: 11px;font-weight: bold;color:#646262">SURNAME/NOM</div>
                <div style="font-size: 12px;margin-top: 2px;color:black;letter-spacing: 2px;font-weight: bold">EDUNOH</div>
            </div>

            <div style="margin-top: 15px">
                <div style="font-size: 11px;font-weight: bold;color:#646262">GIVEN NAMES/PRENOMS</div>
                <div style="font-size: 12px;margin-top: 2px;color:black;letter-spacing: 2px;font-weight: bold">ETIM/ OKON</div>
            </div>

            <div style="margin-top: 15px">
                <div style="font-size: 11px;font-weight: bold;color:#646262">DATE OF BIRTH</div>
                <div style="font-size: 12px;margin-top: 2px;color:black;letter-spacing: 2px;font-weight: bold;width:100%">09 JAN 1989</div>
            </div>


        </div>



    </div>

    <div style="margin-top: 146px;margin-left: 230px;position: absolute;">
        <div style="font-size: 11px;font-weight: bold;color:#646262">SEX/SEXE</div>
        <div style="font-size: 12px;margin-top: 2px;color:black;letter-spacing: 2px;font-weight: bold">M</div>
    </div>


    <div style="margin-top: 200px;margin-left: 110px;position: absolute;width:100%">
        <div style="font-size: 13px;font-weight: bold;color:#000000; letter-spacing: 1px">National Identification Number (NIN)</div>

    </div>

    <div style="margin-top: 214px;margin-left: 75px;position: absolute;width:100%;">

        <div style="font-size: 30px;margin-top: 0px;color:black;letter-spacing: 6px;font-weight: bold;">{{$first_nin}}&nbsp;&nbsp; {{$second_nin}}&nbsp;&nbsp; {{$third_nin}}</div>
    </div>

    <div style="margin-top: 140px;margin-left: 345px;position: absolute;color:black;">
        <div style="font-size: 20px;font-weight: bold;color:#000000">NGA</div>
    </div>

    <div style="margin-top: 165px;margin-left: 330px;position: absolute;color:black;">
        <div style="font-size: 12px;font-weight: bold;color:#000000">ISSUE DATE</div>
    </div>

    <div style="margin-top: 178px;margin-left: 320px;position: absolute;color:black; width: 100%;">
        <div style="font-size: 12px;letter-spacing:1px;color:#000000">06&nbsp;&nbsp; JAN&nbsp;&nbsp; 2023 </div>
    </div>


    <div style="position: absolute">
        <img src="{{$qrcode}}" style="margin-top: 22px;margin-left: 312px;width:108px;height:105px">
    </div>

    <img src="http://iverify.test//image-000.jpg" style="width: 100%;">
</div>
<div class="id-card-tag-strip"></div>
<div class="id-card-hook"></div>
<div class="id-card-holder">
    
</div>

</body>
</html>